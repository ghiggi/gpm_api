...
p = da_precip.plot.imshow(x="along_track", y="cross_track",
                          cmap="Spectral", vmin=0.1)
...
p = da_labels_area.plot.imshow(x="along_track", y="cross_track",
                          cmap="Spectral", vmin=1)
p.cmap.set_under("white")
plt.show()

p = da_labels_area.plot.imshow(x="along_track", y="cross_track",
                          cmap="Spectral", vmin=1, vmax=20)
p.cmap.set_under("white")
plt.show()
...
data_proj = resample_nearest(swath_def, data, area_def,
                             radius_of_influence=20000, fill_value=None)
crs = area_def.to_cartopy_crs()
fig, ax = plt.subplots(subplot_kw=dict(projection=crs))
coastlines = ax.coastlines()  
ax.set_global()
img = plt.imshow(data_proj, transform=crs, extent=crs.bounds,
                 origin='upper')
cbar = plt.colorbar()                       
...
import cartopy.crs as ccrs
fig, ax = plt.subplots(subplot_kw=dict(projection=ccrs.PlateCarree())) 
p = da_precip_subset.plot.pcolormesh(x='lon',y='lat',
                                     ax=ax, 
                                     cmap=cmap, norm=norm, cbar_kwargs=cbar_kwargs,
...
p = da_labels_max.plot.imshow(x="along_track", y="cross_track",
                              cmap="Spectral",
                              vmin=1)
p.cmap.set_under("white")
plt.show()
...
ax.add_geometries([swath_def_bbox_geom], crs=ccrs.PlateCarree(),
                  facecolor = 'orange', edgecolor='black', alpha=0.4)
# ax.set_extent(extent, crs=ccrs.PlateCarree())
ax.stock_img()  
ax.coastlines()  # ax.add_feature(cfeature.COASTLINE.with_scale('110m'), linewidth=0.75)
gl = ax.gridlines(draw_labels=True, linestyle='--')
