...
list_regular_slices = ds_gpm.gpm_api.get_slices_contiguous_scans()
...
np.unique(
    ds_gpm["scanStatus/dataQuality"].isel(frequency=0).data.compute(), return_counts=True
)  # 0, 1, 64, 65
...
# - 64 = 2**6 --> modeStatus not zero
np.unique(
    ds_gpm["scanStatus/dataWarning"].isel(frequency=0).data.compute(), return_counts=True
)  # 0, 8
...
np.unique(
    ds_gpm["scanStatus/missing"].isel(frequency=0).data.compute(), return_counts=True
)  # 0, 3, 9
...
# - 128 = 2**7 --> Spare (always 0)
np.unique(
    ds_gpm["scanStatus/modeStatus"].isel(frequency=0).data.compute(), return_counts=True
)  # 0, 16, 22
...
np.unique(
    ds_gpm["scanStatus/operationalMode"].isel(frequency=0).data.compute(), return_counts=True
)  # 1, 3, 4, 5  -99
# geoWarning
...
np.unique(
    ds_gpm["scanStatus/geoWarning"].isel(frequency=0).data.compute(), return_counts=True
)  # 0, 8
