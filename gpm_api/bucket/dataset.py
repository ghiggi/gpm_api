...
#### Dataset Writers 
def _write_pd_partitioned_dataset(df, 
                                  base_dir, 
                                  fname_prefix,
                                  partitioning,
                                  **writer_kwargs):
    ...
    pyarrow.dataset.write_dataset(table,
                             base_dir=base_dir,
...
def _write_dask_partition(part, 
                          part_index, 
                          base_dir,
                          fname_prefix,
                          partitioning,
    ...
    _write_pd_partitioned_dataset(part, 
                                  base_dir=base_dir, 
...
def _write_dask_partitioned_dataset(df,
                                    base_dir, 
                                    fname_prefix, 
                                    partitioning,
    ...
    for part_index, part in enumerate(df.partitions):
        _write_dask_partition(part=part, 
...
def write_partitioned_dataset(df, 
                              base_dir,
                              partitioning,
                              fname_prefix="part",
                              format="parquet",
    ...
    if isinstance(df, dd.DataFrame):
       _write_dask_partitioned_dataset(df=df, 
                                       base_dir=base_dir, 
                                       fname_prefix=fname_prefix,
                                       partitioning=partitioning,
                                       **writer_kwargs)
    elif isinstance(df, pd.DataFrame): 
       _write_pd_partitioned_dataset(df=df, 
                                     base_dir=base_dir, 
                                     fname_prefix=fname_prefix,
                                     partitioning=partitioning,
                                     **writer_kwargs)
