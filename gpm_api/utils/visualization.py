...
def get_dataset_title(ds, time_idx=None, resolution="m", timezone="UTC", 
    ...
    if add_timestep: 
        ...
        time_str = np.datetime_as_string(timestep,
                                         unit=resolution,
                                         timezone=timezone)
...
def optimize_transect_slices(obj, transect_slices, trim_threshold, variable=None, left_pad=0, right_pad=0):
    #--------------------------------------------------------------------------. 
    # Check variable 
    ...
    if len(idx_above_thr) == 0:
        ...
        raise ValueError("No {trim_variable} value above trim_threshold={trim_threshold}. Try to decrease it.")
    ...
    if len(valid_idx) == 1:
        print("Printing a single profile! To plot a longer profile transect increase `trim_threshold`.")
...
def get_transect_slices(obj, 
    ...
    if isinstance(obj, xr.Dataset):
        if lat is None and lon is None and variable is None:
            raise ValueError("Need to provide 'variable' if passing a xr.Dataset and not specifying 'lat' / 'lon'.")
    ...
    if len(transect_kwargs) != 0: 
        transect_slices = optimize_transect_slices(obj, transect_slices,  
                                                   variable=variable,  
                                                   **transect_kwargs)
    #-------------------------------------------------------------------------.          
...
def plot_profile(da_profile, colorscale=None, ylim=None, ax=None):
    ...
    # Plot 
    p = da_profile.plot.pcolormesh(x=x_direction, y="height", ax=ax, 
...
def plot_transect_line(ds, ax, color="black"):
    ...
    # Draw line 
    ax.plot(lon_startend, lat_startend,
