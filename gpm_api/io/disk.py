...
def _find_daily_filepaths(base_dir, 
                         product, 
                         date, 
                         version=7,
                         start_time=None, 
    ...
    if is_empty(filepaths):
       if verbose: 
           ...
           print(f"The GPM product {product} (V0{version_str}) on date {date} has not been downloaded !")
       return []
    ##------------------------------------------------------------------------.
    ...
    if is_empty(filepaths):
        if verbose: 
            version_str = str(int(version))       
            print(f"No GPM {product} (V0{version_str}) product has been found on disk on date {date} !")
        return []          
