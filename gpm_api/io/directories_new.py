...
def get_local_dir_pattern(product,
                          product_type, 
                          version):
    """
    ...
    # Define pattern
    if product_type == "NRT": 
        dir_structure = os.path.join("GPM", 
                                    product_type, 
                                    product_category,
                                    product) 
...
def get_local_directory(base_dir,
                        product,
                        product_type, 
                        date, 
                        version):
    ...
    """
    dir_structure = get_local_dir_pattern(product,
                                          product_type, 
                                          version)
...
def get_pps_nrt_product_dir(product,  
                            date): 
    """
    ...
    #### - GPM IMERG NRT   
        ...
        dir_structure = os.path.join(folder_name, 
...
def get_pps_rs_product_dir(product,  
                           date, 
                           version): 
    """
    ...
    if version == 7: 
        dir_structure = os.path.join('gpmdata',
    ...
    #### Specify the directory structure for old RS version
        ...
        version_str = "V0" + str(int(version)) 
        dir_structure = os.path.join('gpmallversions',
                                     version_str,
                                     datetime.datetime.strftime(date, '%Y/%m/%d'),
                                     folder_name,
                                     )
...
def get_pps_directory(product, 
    ...
    if product_type == 'NRT':
        ...
        dir_structure = get_pps_nrt_product_dir(product,  
                                                date)
    ...
    #### RS data      
        ...
        # Retrieve directory structure 
        dir_structure = get_pps_rs_product_dir(product,  
                                               date, 
                                               version)
