...
# ----------------------------------------------------------------------------.
def GPM_variables_dict(product, scan_mode, GPM_version=6):
    """
...
def GPM_variables(product, scan_modes=None, GPM_version=6):
    ...
    if len(scan_modes) > 1:
        ...
        for scan_mode in scan_modes:
            GPM_vars = list(
                GPM_variables_dict(
                    product=product, scan_mode=scan_mode, GPM_version=GPM_version
                ).keys()
            )
        ...
        GPM_var_dict = GPM_variables_dict(
            product=product, scan_mode=scan_modes[0], GPM_version=GPM_version
        )
...
def check_scan_mode(scan_mode, product):
    ...
    if scan_mode is not None:
        if (product in ["1B-Ku", "2A-Ku", "2A-ENV-Ku"]) and scan_mode != "NS":
            raise ValueError(
                "For '1B-Ku','2A-Ku'and '2A-ENV-Ku' products, specify scan_mode = 'NS'"
            )
        if (product in ["1B-Ka", "2A-Ka", "2A-ENV-Ka"]) and (
            scan_mode not in ["MS", "HS"]
        ):
            raise ValueError(
                "For '1B-Ka', '2A-Ka' and '2A-ENV-Ka' products, specify scan_mode either 'MS' or 'HS'"
            )
        if (product in ["2A-DPR"]) and (scan_mode not in ["NS", "MS", "HS"]):
            raise ValueError(
                "For '2A-DPR' product, specify scan_mode either 'NS', 'MS' or 'HS'"
            )
        if (product in ["2A-ENV-DPR"]) and (scan_mode not in ["NS", "HS"]):
            raise ValueError(
                "For '2A-ENV-DPR' products, specify scan_mode either 'NS' or 'HS'"
            )
...
def check_variables(variables, product, scan_mode, GPM_version=6):
    ...
    valid_variables = GPM_variables(
        product=product, scan_modes=scan_mode, GPM_version=GPM_version
    )
...
def get_radar_coords(hdf, scan_mode):
...
def get_imerg_coords(hdf, scan_mode):
    hdf_attr = hdf5_file_attrs(hdf)
    lon = hdf[scan_mode]["lon"][:]
    lat = hdf[scan_mode]["lat"][:]
    tt = hdf_attr["FileHeader"]["StartGranuleDateTime"][:-1]
    tt = np.array(np.datetime64(tt) + np.timedelta64(30, "m"), ndmin=1)
    coords = {"time": tt, "lon": lon, "lat": lat}
    return coords
...
def GPM_granule_Dataset(
    ...
    # - For GPM radar products array are stored ('along_track','cross_track')
    if product not in GPM_IMERG_products():
        coords = get_radar_coords(hdf, scan_mode)
    # - For IMERG products
    else:
        coords = get_imerg_coords(hdf, scan_mode)
    ...
    for var in variables:
        ...
        if len(variables_dict[var]["offset_scale"]) == 2:
            da = (
                da / variables_dict[var]["offset_scale"][1]
                - variables_dict[var]["offset_scale"][0]
            )
        ## --------------------------------------------------------------------.
    ...
    for filepath in filepaths:
        ...
        try:
            ...
            if not os.path.exists(filepath):
                ...
                print(
                    "The following file is corrupted and is being removed: {}".format(
                        filepath
                    )
                )
        ...
        if product in GPM_DPR_RS_products():
            ...
            if (
                hdf_attr["JAXAInfo"]["TotalQualityCode"]
                not in DataQualityFiltering["TotalQualityCode"]
            ):
                continue
        # ---------------------------------------------------------------------.
