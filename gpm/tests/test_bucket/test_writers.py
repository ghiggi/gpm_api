...
import polars as pl 
from gpm.bucket.writers import convert_size_to_bytes, write_partitioned_dataset, estimate_row_group_size
...
def test_filesize_conversions():
    ...
    assert 1 == convert_size_to_bytes(1)
...
class TestWriteDaskPartitionedDataset:
    ...
    def test_without_partitions(self, tmp_path):
        ...
        # Assert structure
        assert os.listdir(tmp_path) == ["prefix_dask_partition_0_0.parquet", 
    ...
    def test_with_partitions(self, tmp_path):
        ...
        assert os.listdir(os.path.join(tmp_path, "0")) == ["prefix_dask_partition_0_0.parquet", "prefix_dask_partition_1_0.parquet"]
...
def test_write_polars_partitioned_dataset(tmp_path):
    ...
    row_group_size = 3
    max_file_size = 10
    write_partitioned_dataset(df_pl, 
                            base_dir=tmp_path, 
                            filename_prefix="prefix", 
                            partitions=None, 
                            row_group_size=row_group_size, 
