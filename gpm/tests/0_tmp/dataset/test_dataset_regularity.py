...
ds.isel(along_track=slice(0, 10000)).gpm.plot_map(variable=variable)

###---------------------------------------------------------------------------.
from gpm.utils.checks import get_contiguous_scan_slices, get_discontiguous_scan_slices

ds.gpm.is_regular
ds.gpm.has_regular_timesteps
ds.gpm.has_contiguous_scans
ds.gpm.get_regular_time_slices()
ds.gpm.get_contiguous_scan_slices()

#### Operational checks
gpm.check_regular_timesteps(ds)
gpm.check_contiguous_scans(ds)
gpm.check_valid_geolocation(ds)

###---------------------------------------------------------------------------.
# Mock discontinguous and test plotting
indices = np.arange(0, 1000)
ds.isel(along_track=indices).gpm.plot_map(variable=variable)
indices = np.append(indices[0:200], indices[800:1000])
ds.isel(along_track=indices).gpm.get_contiguous_scan_slices()
ds.isel(along_track=indices).gpm.plot_map(variable=variable)
...
ds.isel(along_track=list_slices[0]).gpm.plot_map(variable=variable)
...
p = da.gpm.plot_map_mesh(edgecolors="r")
...
da.gpm.plot_map_mesh(ax=p.axes)
