...
ds_gpm["precipRateNearSurface"].gpm.plot_map()

xr_obj = ds_gpm
xr_obj.gpm.get_slices_valid_geolocation()  # till 7434
xr_obj.gpm.get_slices_contiguous_scans()  # till 7435
xr_obj.gpm.is_regular
xr_obj.isel(along_track=7434)["lon"]
xr_obj.isel(along_track=7435)["lon"]
ds_gpm.isel(along_track=slice(0, 7435, None))["precipRateNearSurface"].gpm.plot_map()
ds_gpm.isel(along_track=slice(0, 7435, None)).gpm.is_regular
...
xr_obj.gpm.get_slices_contiguous_scans(min_size=2)
xr_obj.gpm.get_slices_contiguous_scans(
    min_size=1
